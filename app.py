...
def run_bot(username, password, welcome_messages, group_ids, delay, poll_interval, custom_name):
    cl = Client()
# --- SESSION BASED LOGIN (NO PASSWORD USED) ---
try:
    if os.path.exists(SESSION_FILE):
        log("🔑 Loading session.json...")
        cl.load_settings(SESSION_FILE)

        # Validate session
        try:
            cl.get_timeline_feed()
            log("✅ Session working — Logged in successfully without password.")
        except:
            log("❌ Session expired — Upload a new session.json file.")
            return

    else:
        log("❌ session.json file missing! Upload a fresh session.json first.")
        return
